...
export function ChallengeSideDrawer({
  ...
  useEffect(() => {
    const fetchStravaIntegration = async () => {
      if (user && isOpen) {
        try {
          ...
          console.error("Error loading Strava integration info in drawer:", err);
        }
      } else {
        setStravaIntegration(null);
      }
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (event.data?.type === 'STRAVA_AUTH_SUCCESS') {
        if (user && user.uid === event.data.userId) {
          try {
            const userDocRef = doc(db, 'users', user.uid);
            ...
            console.error("Failed to update user doc with Strava data inside drawer:", dbErr);
            setProfileError("Erro ao salvar dados do Strava no banco de dados.");
          }
        }
      }
    };
  ...
  const handleConnectStrava = async () => {
    if (!user) {
      setProfileError("Você precisa estar logado para conectar com o Strava.");
      return;
    }
    
    setIsStravaLoading(true);
    try {
      const response = await fetch(`/api/strava/auth-url?userId=${user.uid}`);
      if (!response.ok) {
      ...
      if (data.url === "DEMO_MODE") {
        ...
        setProfileError(null);
        alert("Modo Simulação Strava ativado! Clique em 'Sincronizar Atividades' para carregar corridas simuladas.");
      } else {
        const width = 600;
        const height = 700;
        ...
        if (!popup) {
          setProfileError("O bloqueador de popups impediu a janela do Strava de abrir. Por favor, permita popups para este site.");
        }
      }
    } catch (err: any) {
      console.error("Connect strava failed:", err);
      setProfileError(`Erro ao conectar com o Strava: ${err.message}`);
    } finally {
  ...
  const handleDisconnectStrava = async () => {
    ...
    if (confirm("Deseja realmente desconectar sua conta do Strava?")) {
      try {
        ...
        setProfileError("Erro ao desconectar Strava no banco de dados.");
      }
    }
  };

  const handleSyncStrava = async () => {
    if (!user || !stravaIntegration) return;
    ...
    try {
      ...
      if (stravaActs.length === 0) {
        alert("Nenhuma atividade recente encontrada no seu Strava.");
        return;
      }
      ...
      if (newActs.length === 0) {
        alert(`Sincronizado! Seus treinos recentes do Strava já estão todos registrados no Desafio.`);
        return;
      }
      ...
      alert(`Sucesso! Importamos e registramos automaticamente ${addedCount} novos treinos do Strava.`);
    } catch (err: any) {
      console.error("Sync error:", err);
      setProfileError(`Falha na sincronização automatizada: ${err.message}`);
    } finally {
      setIsSyncing(false);
    }
  };
            ...
            <div className="p-6 pt-10 border-b border-slate-900 flex flex-col gap-3 bg-slate-900/20 w-full">
              ...
              {isEditingProfile ? (
                <div className="space-y-3 w-full">
                  ...
                  <div className="bg-slate-950/60 border border-slate-900 rounded-xl p-3 my-1 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-extrabold text-orange-500 uppercase tracking-wider flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse" />
                        Sincronização Strava
                      </span>
                      {stravaIntegration ? (
                        <span className="text-[9px] bg-emerald-950/50 text-emerald-400 border border-emerald-900/40 px-1.5 py-0.5 rounded-md font-bold uppercase tracking-wide">
                          Ativo
                        </span>
                      ) : (
                        <span className="text-[9px] bg-slate-900 text-slate-500 border border-slate-800 px-1.5 py-0.5 rounded-md font-bold uppercase tracking-wide">
                          Inativo
                        </span>
                      )}
                    </div>
                    ...
                    {stravaIntegration ? (
                      <div className="space-y-2">
                        <div className="text-[11px] text-slate-300 leading-normal font-semibold">
                          Atleta: <span className="text-orange-400 notranslate" translate="no">{stravaIntegration.athlete?.firstname || 'Atleta'} {stravaIntegration.athlete?.lastname || ''}</span>
                        </div>
                        <div className="flex gap-1.5">
                          <button
                            ...
                            disabled={isSyncing}
                            className="flex-1 py-1 px-2 bg-orange-600 hover:bg-orange-500 disabled:bg-orange-800 text-white font-extrabold text-[9px] rounded-lg cursor-pointer transition flex items-center justify-center gap-1 uppercase tracking-wider"
                          >
                            {isSyncing ? (
                              <>
                          ...
                          <button
                            ...
                            className="py-1 px-2 bg-red-950/40 hover:bg-red-900/40 text-red-400 hover:text-white rounded-lg font-bold text-[9px] cursor-pointer transition border border-red-900/30"
                          >
                            Desconectar
                          </button>
                        </div>
                      ...
                      <div className="space-y-1.5">
                        <p className="text-[10px] text-slate-400 leading-relaxed font-sans">
                          Sincronize suas corridas, caminhadas e pedaladas automaticamente.
                        </p>
                        <button
                          type="button"
                          onClick={handleConnectStrava}
                          disabled={isStravaLoading}
                          className="w-full py-1.5 bg-orange-600 hover:bg-orange-500 disabled:bg-orange-800 text-white font-extrabold text-[9px] rounded-lg cursor-pointer transition flex items-center justify-center gap-1.5 uppercase tracking-wider"
                        >
                          {isStravaLoading ? (
                            <>
                              <span className="w-2.5 h-2.5 border border-white border-t-transparent rounded-full animate-spin" />
                            ...
                            <span>Conectar Conta Strava</span>
                          )}
                        </button>
                      </div>
                    )}
